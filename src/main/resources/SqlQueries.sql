...
select name, city from girls;
select name, city from boys where city ='Boston';
select city, target, sales from offices where region='Восток' order by city;
select avg(target), avg(sales) from offices where region='Запад';
select hair, eyes from coloris;
select avg(sales) from salesreps where empl_num = 107;
select name, hire_date, sales from salesreps where sales > 200;
SELECT CITY, REGION, SALES FROM OFFICES;
SELECT CITY, REGION, (TARGET-SALES) FROM OFFICES;
COL CITY FORMAT A18;
...
SELECT CITY, 'something', SALES FROM OFFICES;
...
TO_DATE('08.06.1990','DD/MM/YYYY'), TO_DATE('29.06.1990','DD/MM/YYYY'), TO_DATE('04.06.1990','DD/MM/YYYY'));
...
OR (MANAGER IS NULL AND HIRE_DATE >= TO_DATE('01.05.88','DD/MM/YYYY')) OR (SALES > QUOTA AND NOT SALES > 60.0);
SELECT CITY, REGION, SALES FROM OFFICES ORDER BY REGION, CITY;
SELECT CITY, REGION, SALES FROM OFFICES ORDER BY SALES DESC;
SELECT CITY, REGION, (SALES - TARGET) FROM OFFICES ORDER BY 3 ASC;
...
SELECT MFR_ID, PRODUCT_ID FROM PRODUCTS WHERE PRICE > 500.00 UNION 
SELECT DISTINCT MFR, PRODUCT FROM ORDERS WHERE AMOUNT > 4.00;
SELECT MFR_ID, PRODUCT_ID FROM PRODUCTS WHERE PRICE > 500.00 UNION ALL
SELECT DISTINCT MFR, PRODUCT FROM ORDERS WHERE AMOUNT > 4.00 ORDER BY 1,2;
